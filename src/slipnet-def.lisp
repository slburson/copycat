...
; defined in the file "sliplinks.l".
  ...
  (setq plato-leftmost
	(make-instance 'slipnode
	  ...
	  :description-tester
	  (lambda (object)
	    (and (not (send object :spans-whole-string?))
  ...
  (setq plato-middle
	  ...
	  :description-tester
	  (lambda (object)
 	    (let ((left-neighbor (send object :ungrouped-left-neighbor))
 	          (right-neighbor (send object :ungrouped-right-neighbor)))
  ...
  (setq plato-first
	(make-instance 'slipnode
	  ...
	  :description-tester
	  (lambda (object)
            (eq (send object :get-descriptor plato-letter-category)
		plato-a))))
  ...
  (setq plato-predgrp
	  ...
	  (lambda (letter-category)
	    (send letter-category :get-related-node plato-predecessor))))
  ...
  (setq plato-succgrp
	  ...
	  :lateral-nonslip-links '(succgrp-successor-link
 	  ...
 	  :iterate-group
	  (lambda (letter-category)
	    (send letter-category :get-related-node plato-successor))))
  ...
  (loop for slipnode in *slipnet* do
        ...
        (send slipnode :set-incoming-links
	               (mapcar #'symbol-value (send slipnode :incoming-links)))
        (send slipnode :set-category-links
	               (mapcar #'symbol-value (send slipnode :category-links)))
        (send slipnode :set-instance-links
	               (mapcar #'symbol-value (send slipnode :instance-links)))
        (send slipnode :set-has-property-links
	               (mapcar #'symbol-value (send slipnode :has-property-links)))
        (send slipnode :set-lateral-slip-links
	               (mapcar #'symbol-value (send slipnode :lateral-slip-links)))
        (send slipnode :set-lateral-nonslip-links
	               (mapcar #'symbol-value (send slipnode :lateral-nonslip-links))))
