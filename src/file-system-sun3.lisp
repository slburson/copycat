...
(defflavor lisp-file
  (l-version lbin-version last-l-load-time last-lbin-load-time)
    :gettable-instance-variables
    :settable-instance-variables
    :initable-instance-variables)
...
(defun defsys (name list-of-filenames &aux list-of-files)
  ...
  (setq list-of-files
	(loop for filename in list-of-filenames 
	      collect (make-instance 'lisp-file 
			  ...
			  :lbin-version (string-append filename ".lbin"))))
  ...
  (loop for file in list-of-files do
	...
	(send file :set-last-lbin-load-time 0))
...
(defmethod (lisp-file :reload) (&aux file-to-load last-load-time)
  ; First see whether to consider the .l version or the .lbin version.  
  ; If both exist, load most recent.
  (cond ((and (probe-file l-version) (probe-file lbin-version))
	 (if* (< (file-write-date l-version)
		 (file-write-date lbin-version)) ; .lbin version is more recent
	  then (setq file-to-load lbin-version)
	       (setq last-load-time last-lbin-load-time)
	  else (setq file-to-load l-version)
      	       (setq last-load-time last-l-load-time)))
	((probe-file l-version) 
	 (setq file-to-load l-version)
         (setq last-load-time last-l-load-time))
	((probe-file lbin-version) 
	 (setq file-to-load lbin-version)
         (setq last-load-time last-lbin-load-time))
  ...
  (if* (< (+ last-load-time 15) (file-write-date file-to-load))
   then (if* (eq file-to-load lbin-version) 
	 then (send self :set-last-lbin-load-time (get-universal-time))
...
; If the .l version is more recent than the .lbin version or if the .lbin 
; version doesn't exist, then compile the file and load the compiled version.
; Otherwise, load the .lbin version.
  (cond ((and (probe-file l-version) (probe-file lbin-version))
	 (if* (< (file-write-date lbin-version)
  ...
  (if* compile? 
   then (compile-file l-version)  
        (load lbin-version)
        (send self :set-last-lbin-load-time (get-universal-time))
   else ; Load the .lbin file if it hasn't been loaded since its 
        ...
        (if* (< (+ last-lbin-load-time 15) (file-write-date lbin-version))
         then (load lbin-version)
              (send self :set-last-lbin-load-time (get-universal-time)))))
